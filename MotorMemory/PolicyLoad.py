...
def creat_policy(env, inputs, device, policy_func, optimizer_mod, learning_rate) :
    vision_mask = [1]
    proprio_mask = [1]
    task_mask = [1]
    output_mask = [1]
    ...
    module_sizes = [256]#[128]
    spectral_scaling = 1
    # input sparsity
    vision_dim = np.arange(env.get_vision().shape[1])
    proprio_dim = np.arange(env.get_proprioception().shape[1]) + vision_dim[-1] + 1
    task_dim = np.arange(inputs.shape[2]) + proprio_dim[-1] + 1
    policy = policy_func(env.observation_space.shape[0] + inputs.shape[2], module_sizes, env.n_muscles,
